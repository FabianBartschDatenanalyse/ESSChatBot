  ...
  try {
    ...
    for (const col of allColumns) {
      if (col === 'gndr') {
        ...
        whereClauses.push(
          `"${col}" NOT IN ('7','8','9','55','66','77','88','99','555','777','888','999','9999')`
        );
      }
    }
    ...
    if (!queryData?.length) {
      return {
        error: 'No data available for regression after querying.',
        sqlQuery: query,
      };
    }
    ...
    const featureTensor = tf.tensor2d(
      processedFeatures,
      [cleanData.length, features.length],
      'float32'
    );
    const targetTensor = tf.tensor2d(
      cleanData.map(r => [r[target]]),
      [cleanData.length, 1],
      'float32'
    );
    ...
    const finalNames = features.map(n =>
      n === 'gndr' ? 'female' : n === 'agea' ? 'age_centered' : n
    );
    finalNames.forEach((name, i) => {
      coefficients[name] = kernel[i][0];
    });
