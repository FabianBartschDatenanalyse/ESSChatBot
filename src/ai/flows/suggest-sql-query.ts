...
const prompt = ai.definePrompt({
  ...
  **CRITICAL RULES:**
  ...
  5.  **Filtering Missing Values:** When aggregating data (e.g., with AVG, COUNT), you MUST exclude rows with missing or invalid data. The codebook specifies missing values with codes like 77 (Refusal), 88 (Don't know), and 99 (No answer). These are stored as TEXT, so you MUST compare them as strings. Always include a \`WHERE\` clause to filter these out (e.g., \`WHERE trstprl NOT IN ('77', '88', '99')\`).
